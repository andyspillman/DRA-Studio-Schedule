...
sub buildtable {
    ...
    until ( $timeblock > $maxtimeblock )
    {                ##check for how many timeblocks there actually are
        print "<tr>";
        until ( $tablecol > 7 ) {
            makecell();
            $tablecol++;
...
sub makecell {
    ...
    if ( $timeblocklength == 1 ) {
        ...
        @timeblockname =
          ( "", "8 am - 12 pm", "12 pm - 4 pm", "4-pm - 8 pm", "8 pm - 12 am",
          );
    }
    print "<td>";
    if ( $tablecol == 0 ) {
...
#days, in which case nothing happens and the cell is blank
    if (
        my $timeblockowner = DatabaseUtil::getowner(
            $main::weekstart->clone->add( days => $tablecol - 1 ), $timeblock
        )
      )
    {    ###change to include $celldate instead???
        $celldate =
          $main::weekstart->clone->add( days => $tablecol - 1 )->ymd('');
...
#cases
        ...
        {    ### is the paren necessary?
            if ($main::faculty) {
                dropdown('-shared-');
### same note as above
                print startform( -action => "" )
                  . textarea(
                    -name      => 'shared_names',
                    -default   => $1,
                    -rows      => 5,
                ...
                foreach (@timeblockowners) {
                    if ( $_ && !( $_ eq 'shared' ) )
                    { ###perhaps a better regex call would remove the need for this
                        print p( $main::labels{$_} );
                    }
                }
                if ( $main::username ~~ @timeblockowners )
                {     ###give user the option to give up if user is an owner
                    print a(
                        {
                            -href => url( -relative => 1 )
                              . "?room=$main::room&weekstart=$main::weekstart&targetdate=$celldate&timeblock=$timeblock&newowner=-open-"
                        },
                        "give up"
                    );
                }
        ...
        elsif ( ( $timeblockowner eq '-open-' )
            && $main::labels{$main::username} )
        {
            print a(
                {
                    -href => url( -relative => 1 )
                      . "?room=$main::room&weekstart=$main::weekstart&targetdate=$celldate&timeblock=$timeblock&newowner=$main::username"
                },
                "take"
            );
...
#
    ...
    print popup_menu(
        ...
        -labels  => \%main::labels,
        -onChange =>
"window.location.href='schedule.cgi?room=$main::room&weekstart=$main::weekstart&targetdate=$celldate&timeblock=$timeblock&newowner='+this.value",
