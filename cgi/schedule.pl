...
if(!(url_param('room')&&(url_param('room') ~~ @{DatabaseUtil::roomnames()}))){
...
if (!url_param('weekstart')){#if page is opened without ?weekstart=, assume current week
...
###week navigation under table
    print p(a({-href=>url(-relative=>1)."?room=$room&weekstart=".$weekstart->clone()->subtract(weeks=>1)},
          "Previous Week").a({-href=>url(-relative=>1)."?room=$room&weekstart=".
            $weekstart->clone()->add(weeks=>1)}, "Next Week"));

    if(!(($weekstart<$curr_dt)&&($curr_dt<$weekend))){
      print p(a({-href=>url(-relative=>1)."?room=$room"}, "Current Week"));
    };
