...
# In addition to the modules eliclty included, LWP::UserAgent
...
if ( defined($cas_ticket)
    && $cas_ticket =~ m/^ST-\d+-[A-Za-z0-9]+-casprd\d\d.uits.iu.edu$/ )
{

    # Retrieve the contents of the validation response
    my $req = new HTTP::Request( 'GET',
"https://cas.iu.edu/cas/validate?cassvc=$cassvc&casticket=$cas_ticket&casurl=$casurl"
    );
...
# It means that CAS generated a ticket which we were unable to validate.  Bad passwords and invalid usernames  should be stopped at the CAS server.
        print $cgi->header();
  ...
  # we need to redirect the user to the CAS logon page.
    print $cgi->header( -Refresh =>
          "0; URL=https://cas.iu.edu/cas/login?cassvc=$cassvc&casurl=$casurl" );
